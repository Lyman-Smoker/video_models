...
def load_model(model_path):
    ...
    return c3d, regressor, optimizer_state_dict
...
c3d, score_regressor, optimizer_state_dict = load_model('./models/'+ checkpoint + '.pth')
c3d = torch.nn.DataParallel(c3d, device_ids=device_ids)
c3d = c3d.cuda(device=device_ids[0])
# print('c3d is in {}'.format(next(c3d.parameters()).device))
...
optimizer = optim.SGD(
    params=(list(c3d.parameters()) + list(score_regressor.parameters())),
    lr=LEARNING_RATE,
    momentum=MOMENTUM
)
...
def train(epoch, save_interval):
    ...
    for i in range(29, 29 + epoch):
        ...
        for video_clips, label in tqdm(trainset_loader):
            ...
            # print('video_clips[0].shape:', video_clips[0].shape)
            ...
            # print('video_feature_average.shape: ', video_feature_average.shape)
            pred_score = score_regressor(video_feature_average)
            final_loss = loss(pred_score, label)
            # print('loss type:', type(final_loss.item()))
            optimizer.zero_grad()
            final_loss.backward()
            optimizer.step()
